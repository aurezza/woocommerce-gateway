 ...
 * 
 * @author Aurezza - <aurezza@chubachu.com>
 * @version 1.0.1
 */

/**
 * Plugin Name: DragonPay Plugin for Woocommerce
 * Description: A payment gateway plugin/extension for WooCommerce. Modified from Dennis Paler's original plugin to suit current client needs
 */

function wcdragonpay_woocommerce_fallback_notice() {
    $message = '<div class="error">';
    $message .= '<p>' . __( 'This plugin depends on the latest version of <a href="http://wordpress.org/extend/plugins/woocommerce/">WooCommerce</a> to work!' , 'wcdragonpay' ) . '</p>';
    $message .= '</div>';
    echo $message;
}
...
function wcdragonpay_gateway_load() {
    ...
    class WC_Dragonpay_Gateway extends WC_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->id = 'dragonpay';
            $this->icon = plugins_url( 'images/dragonpay_logos.png', __FILE__ );
        ...
        public function init_form_fields() {
            ...
            $this->form_fields = array(
                ...
                'title' => array(
                    ...
                    'type' => 'text',
                    'description' => __( 'Title of the gateway/option which the user sees during checkout.', 'wcdragonpay' ),
                    'default' => __( 'Dragonpay Philippines Online Payment', 'wcdragonpay' )
                ),
                'description' => array(
                    'title' => __( 'Description', 'wcdragonpay' ),
                    'type' => 'textarea',
                    'description' => __( 'The description the user sees during checkout.', 'wcdragonpay' ),
                    'default' => __( 'Pay with Dragonpay Philippines Online Payment', 'wcdragonpay' )
                ),
                'merchant_id' => array(
                    'title' => __( 'Merchant ID', 'wcdragonpay' ),
                    'type' => 'text',
                    'description' => __( 'Enter your Dragonpay Merchant ID.', 'wcdragonpay' ) . ' ' . sprintf( __( 'Please refer to your DragonPay representative or you can to get this information in: %sDragonay Account%s.', 'wcdragonpay' ), '<a href="https://www.dragonpay.ph/" target="_blank">', '</a>' ),
                    'default' => ''
                ),
                'password' => array(
                    'title' => __( 'Password', 'wcdragonpay' ),
                    'type' => 'text',
                    'description' => __( 'Enter your Dragonpay Password.', 'wcdragonpay' ) . ' ' . sprintf( __( 'Please refer to your DragonPay representative or you can to get this information in: %sDragonpay Account%s.', 'wcdragonpay' ), '<a href="https://www.dragonpay.ph" target="_blank">', '</a>' ),
                    'default' => ''
                )
            );
        }
